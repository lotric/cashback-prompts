...
| Раздел | Содержание |
|---|---|
| Процесс и контекст использования | Проектирование модели данных ядра кэшбэка. Этап после BRD и Sequence, до описания контрактов API. |
| Цель создания | Зафиксировать структуру данных: сущности, атрибуты, первичные и внешние ключи, кардинальность связей и справочники, необходимые для реализации сценариев. |
| Что становится определено | Сущности `Transaction`, `CashbackRule`, `CashbackAccrual`, `Limit`, `Currency`, `Commission`, `Payout`, `StatusHistory`, `PayoutAccrual`; их атрибуты, связи, ограничения, приведение к 3НФ. |
| Пользователи артефакта | DBA, архитектор данных, разработчики, аналитики, QA. Используют для создания схемы БД, миграций и проверки корректности хранения данных. |
| Использование в дальнейшем | Основа для OpenAPI: схемы `components.schemas` строятся на сущностях ER. Используется при проектировании миграций, индексов и отчётности. |
| Последствия отсутствия | Схема БД будет проектироваться «на ходу», появятся дубли, несогласованные связи и потеря данных о статусах и лимитах. Вырастет стоимость изменений и риск ошибок в расчётах. |
